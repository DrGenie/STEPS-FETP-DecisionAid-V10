/* ===================================================
   ...
   WTP based benefits, sensitivity, Copilot and exports
   =================================================== */
...
function autoEnablersAndRisks(cfg) {
  const tier = String(cfg.programTier || cfg.tier || "frontline");
  const mentorship = String(cfg.mentorship || "medium");
  const delivery = String(cfg.deliveryMode || cfg.delivery || "blended");

  const commonEnablers = [
    "Leverage NCDC stewardship and clear accreditation criteria.",
    "Standardised curriculum, assessment, and mentorship protocols.",
    "Phased expansion using hubs and peer-learning across sites."
  ];
  const commonRisks = [
    "Mentor availability constraints as scale grows.",
    "Uneven site readiness and supervision quality across institutions.",
    "Delays in incentive payments and retention challenges."
  ];

  const tierEnablers = {
    frontline: [
      "Use short-cycle cohorts to respond quickly to demand spikes.",
      "Deploy simple QA checks (logbooks, supervision audits) to protect quality at scale."
    ],
    intermediate: [
      "Align placements with state/district surveillance priorities and outbreak rosters.",
      "Build a train-the-mentor pipeline to stabilise supervision capacity."
    ],
    advanced: [
      "Secure multi-year funding commitments and protected time for advanced mentorship.",
      "Partner with academic/public health institutes for specialised modules and analytics support."
    ]
  };

  const tierRisks = {
    frontline: [
      "High throughput can dilute mentoring time per trainee if supervision is not ring-fenced.",
      "Operational disruptions (deployments) can interrupt learning cycles."
    ],
    intermediate: [
      "Service delivery workload may crowd out training time without protected-release agreements.",
      "Complex coordination across sites can slow cohort starts."
    ],
    advanced: [
      "Long training duration increases attrition risk without career-path incentives.",
      "Higher specialisation requirements can bottleneck faculty and mentors."
    ]
  };

  const mentorshipNotes = {
    low: { en: ["Keep expectations explicit: minimum touchpoints, escalation routes, and triage of mentor time."],
           rk: ["Low-touch support may reduce completion and perceived programme value if not paired with safeguards."]},
    medium: { en: ["Define monthly supervision cadence and clear mentor-to-trainee ratios."],
              rk: ["If mentor capacity is tight, medium intensity may still trigger shortfalls in peak periods."]},
    high: { en: ["High-intensity mentoring can lift outcomes if mentor pipeline and QA are resourced."],
            rk: ["High-intensity support can become infeasible quickly without expanding mentor pool."]}
  };

  const deliveryNotes = {
    online: { en: ["Use online delivery to reach smaller sites and reduce travel frictions."],
              rk: ["Purely online delivery can weaken field supervision unless paired with site-based preceptors."]},
    inperson: { en: ["In-person modules support cohort identity and skills practice."],
                rk: ["Travel/logistics costs and scheduling can delay delivery across states."]},
    blended: { en: ["Blended delivery balances reach with periodic in-person skill-building."],
               rk: ["Requires reliable connectivity and coordination across learning modalities."]}
  };

  const en = []
    .concat(commonEnablers)
    .concat((tierEnablers[tier] || []))
    .concat((mentorshipNotes[mentorship] ? mentorshipNotes[mentorship].en : []))
    .concat((deliveryNotes[delivery] ? deliveryNotes[delivery].en : []))
    .filter(Boolean);

  const rk = []
    .concat(commonRisks)
    .concat((tierRisks[tier] || []))
    .concat((mentorshipNotes[mentorship] ? mentorshipNotes[mentorship].rk : []))
    .concat((deliveryNotes[delivery] ? deliveryNotes[delivery].rk : []))
    .filter(Boolean);
...
function getConfigFromForm() {
  ...
  let exportEnablers = enablersEl ? (enablersEl.value || "").trim() : "";
  let exportRisks = risksEl ? (risksEl.value || "").trim() : "";
  // Auto-fill if empty (still editable by user)
  if (!exportEnablers || !exportRisks) {
    const auto = autoEnablersAndRisks({ programTier: programTier, mentorship: mentorship, deliveryMode: deliveryMode });
    if (!exportEnablers) exportEnablers = auto.enablers;
    if (!exportRisks) exportRisks = auto.risks;
    if (enablersEl && !enablersEl.value) enablersEl.value = exportEnablers;
    if (risksEl && !risksEl.value) risksEl.value = exportRisks;
  }
...
function refreshSensitivityTables() {
  ...
  scenarios.forEach(({ label, scenario }) => {
    ...
    let combinedBenefit = effectiveWtp; // Perceived programme value only (epi removed)
...
function buildScenarioJsonForCopilot() {
  // Build a comprehensive structured prompt context including:
  // (1) current scenario outputs (if computable)
  // (2) recommended scenarios (Top 5, model-generated)
  // (3) user-saved scenarios
  // (4) Enablers + Risks (auto-filled)
  // (5) Assumptions
  const cfg = getConfigFromForm();
  let current = null;
  try { current = computeScenario(cfg); } catch(e) {}

  const rec = [];
  try {
    const pool = generateRecommendedScenarioPool();
    const items = computeTopScenarioItems(pool, { metric: "balanced", feasibleOnly: false, selectedOnly: false, limit: 5 });
    items.forEach((it)=>rec.push(it));
  } catch(e) {}

  const saved = Array.isArray(appState.savedScenarios) ? appState.savedScenarios.slice() : [];

  const auto = autoEnablersAndRisks({ programTier: cfg.programTier, mentorship: cfg.mentorship, deliveryMode: cfg.deliveryMode });

  const enablers = (cfg.exportEnablers || "").trim() || auto.enablers;
  const risks = (cfg.exportRisks || "").trim() || auto.risks;
  ...
  function tableMarkdown(rows) {
    const header = ["Scenario","Tier","Endorsement","Perceived programme value","Economic cost (total)","Net benefit","BCR","Feasibility","Mentor shortfall"];
    const sep = header.map(()=> "---");
    const lines = [];
    lines.push("| " + header.join(" | ") + " |");
    lines.push("| " + sep.join(" | ") + " |");
    rows.forEach((s)=>{
      const feas = s.isFeasible ? "Within capacity" : "Requires expansion";
      lines.push("| " + [
        (s.displayName || s.name || "Scenario"),
        String(s.programTierLabel || s.tierLabel || s.programTier || ""),
        fmt(s.endorsementRate,1) + "%",
        fmt(s.perceivedProgrammeValueTotal,0),
        fmt(s.totalEconomicCost,0),
        fmt(s.netBenefit,0),
        fmt(s.bcr,2),
        feas,
        fmt(s.mentorShortfall,0)
      ].join(" | ") + " |");
    });
    return lines.join("\n");
  }

  const recMd = rec.length ? tableMarkdown(rec.map((x)=>Object.assign({}, x, {displayName: x.recommendedLabel || x.displayName || x.name || "Recommended"}))) : "No recommended scenarios were generated.";
  const savedMd = saved.length ? tableMarkdown(saved) : "No user-saved scenarios yet.";

  return {
    currentScenario: current,
    recommendedTop5Markdown: recMd,
    savedScenariosMarkdown: savedMd,
    enablers,
    risks,
    assumptions: buildAssumptionsForScenario(cfg)
  };
}
...
function buildCopilotChatgptPromptText(target = "copilot") {
  const pack = buildScenarioJsonForCopilot();
  const mode = target === "chatgpt" ? "ChatGPT" : "Microsoft Copilot";

  const current = pack.currentScenario;
  const currentText = current ? (
`CURRENT CONFIGURATION (computed outputs)\n` +
`- Scenario: ${current.displayName || current.name || "Current configuration"}\n` +
`- Tier: ${String(current.programTierLabel || current.tierLabel || current.programTier || "-")}\n` +
`- Endorsement: ${Number(current.endorsementRate||0).toFixed(1)}%\n` +
`- Perceived programme value (total): ${Number(current.perceivedProgrammeValueTotal||0).toLocaleString()}\n` +
`- Economic cost (total): ${Number(current.totalEconomicCost||0).toLocaleString()}\n` +
`- Net benefit: ${Number(current.netBenefit||0).toLocaleString()}\n` +
`- BCR: ${Number(current.bcr||0).toFixed(2)}\n` +
`- Feasibility: ${current.isFeasible ? "Within capacity" : "Requires capacity expansion"}\n` +
`- Mentor shortfall: ${Number(current.mentorShortfall||0).toLocaleString()}\n`
) : `CURRENT CONFIGURATION\n- Could not compute outputs. Please check required inputs and try again.\n`;

  const instructions =
`\nTASK\nWrite a decision-ready, two-page policy brief for senior leadership. Use plain policy language (avoid technical jargon).\n\nREQUIREMENTS\n1) Start with a one-paragraph executive summary stating the preferred option and why (value-for-money + feasibility).\n2) Include a compact comparison table.\n3) Clearly distinguish: (A) Recommended scenarios (Top 5, model-generated) vs (B) User-saved scenarios (user-defined).\n4) Include a short feasibility paragraph explaining mentor requirements and the mentor shortfall.\n5) Include Enablers and Risks sections using the bullets provided.\n6) Add a short 'What would change the recommendation?' sensitivity paragraph (thresholds / key drivers).\n\nOUTPUT FORMAT\n- Use clear headings and short paragraphs.\n- Include the provided tables verbatim (do not change columns).\n`;

  const prompt =
`${currentText}\nRECOMMENDED SCENARIOS (TOP 5 — model-generated, independent of what the user saved)\n${pack.recommendedTop5Markdown}\n\nUSER-SAVED SCENARIOS (user-defined)\n${pack.savedScenariosMarkdown}\n\nENABLERS\n${pack.enablers}\n\nRISKS\n${pack.risks}\n\nASSUMPTIONS (from STEPS settings)\n${JSON.stringify(pack.assumptions, null, 2)}\n${instructions}`;
...
function buildPrompt(target) {
  ...
  // Populate prompt previews (full, not truncated)
  try {
    const copilotArea = document.getElementById("copilot-prompt-output");
    if (copilotArea) {
      const t1 = buildCopilotChatgptPromptText("copilot");
      const t2 = buildCopilotChatgptPromptText("chatgpt");
      copilotArea.value = "INTERPRETED RESULTS (from STEPS)\n" + "- Use the tables above for quick comparison.\n\n" + "PROMPT FOR MICROSOFT COPILOT\n" + t1 + "\n\n" + "PROMPT FOR CHATGPT\n" + t2;
    }
  } catch(e) {}

  // Wire copy/open buttons with consistent toasts and safeguards
  function copyToClipboard(text, okMsg) {
    if (!text) return Promise.reject(new Error("empty"));
    return navigator.clipboard.writeText(text).then(()=>{ showToast(okMsg || "Copied to clipboard.", "success"); });
  }

  function requireAtLeastOneScenario() {
    if (!appState.savedScenarios || !appState.savedScenarios.length) {
      showToast("Save at least one scenario before copying prompts.", "error");
      return false;
    }
    return true;
  }

  const btnCopyCopilot = document.getElementById("copilot-copy-btn");
  const btnCopyChatgpt = document.getElementById("chatgpt-copy-btn");
  const btnDownloadPrompts = document.getElementById("briefing-download-btn");
  const btnOpenCopilot = document.getElementById("copilot-open-and-copy-btn");
  const btnOpenChatgpt = null;

  if (btnCopyCopilot) btnCopyCopilot.addEventListener("click", ()=> {
    if (!requireAtLeastOneScenario()) return;
    const t = buildCopilotChatgptPromptText("copilot");
    copyToClipboard(t, "Copilot prompt copied.").catch(()=>showToast("Could not copy prompt.", "error"));
  });

  if (btnCopyChatgpt) btnCopyChatgpt.addEventListener("click", ()=> {
    if (!requireAtLeastOneScenario()) return;
    const t = buildCopilotChatgptPromptText("chatgpt");
    copyToClipboard(t, "ChatGPT prompt copied.").catch(()=>showToast("Could not copy prompt.", "error"));
  });

  if (btnOpenCopilot) btnOpenCopilot.addEventListener("click", ()=> {
    if (!requireAtLeastOneScenario()) return;
    const t = buildCopilotChatgptPromptText("copilot");
    copyToClipboard(t, "Prompt copied. Opening Copilot…").then(()=>{ window.open("https://copilot.microsoft.com/", "_blank", "noopener"); }).catch(()=>showToast("Could not open Copilot.", "error"));
  });

  /* no ChatGPT open button in UI */
  if (false) {};
//("click", ()=> {
    if (!requireAtLeastOneScenario()) return;
    const t = buildCopilotChatgptPromptText("chatgpt");
    copyToClipboard(t, "Prompt copied.").then(()=>{ window.open("https://chat.openai.com/", "_blank", "noopener"); }).catch(()=>showToast("Could not open ChatGPT.", "error"));
  });

  if (btnDownloadPrompts) btnDownloadPrompts.addEventListener("click", ()=> {
    if (!requireAtLeastOneScenario()) return;
    const payload = {
      copilot: buildCopilotChatgptPromptText("copilot"),
      chatgpt: buildCopilotChatgptPromptText("chatgpt")
    };
    try {
      const blob = new Blob([JSON.stringify(payload, null, 2)], {type:"application/json;charset=utf-8"});
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      a.download = "STEPS_prompts.json";
      document.body.appendChild(a);
      a.click();
      a.remove();
      showToast("Prompts downloaded.", "success");
    } catch(e) {
      showToast("Could not download prompts.", "error");
    }
  });
...
/* ===========================
   Configuration status blurb
   =========================== */

function updateConfigStatusBlurb() {
  const el = document.getElementById("config-status-blurb");
  if (!el) return;
  let s = null;
  try {
    const cfg = getConfigFromForm();
  ...
  const nb = Number(s.netBenefit || 0);
  const end = Number(s.endorsementRate || 0);
  const feasible = !!s.isFeasible;
  const shortfall = Number(s.mentorShortfall || 0);

  let label = "";
  const reasons = [];

  if (nb < 0) {
    label = "Challenging configuration";
    reasons.push("WTP/value is below cost");
  } else if (!feasible) {
    label = "Promising configuration";
    reasons.push("requires capacity expansion");
  } else {
    label = "Strong configuration";
    reasons.push("positive net benefit within current capacity");
  }

  if (end < 50) reasons.push("low endorsement");
  if (shortfall > 0) reasons.push(`mentor shortfall (${shortfall})`);

  el.textContent = `${label} (${reasons.join("; ")}).`;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Ensure sensible defaults (requested)
  try {
    const horizonEl = document.getElementById("planning-horizon-config");
    const cohortsEl = document.getElementById("cohorts");
    if (horizonEl && (!horizonEl.value || Number(horizonEl.value) !== 1)) horizonEl.value = "1";
    if (cohortsEl && (!cohortsEl.value || Number(cohortsEl.value) !== 5)) cohortsEl.value = "5";
  } catch(e) {}
